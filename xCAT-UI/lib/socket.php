...
class Net_Socket extends PEAR {
	...
	function readAll()
	{
		...
		while (!feof($this->fp)) {
			...
			if(preg_match($mixedpattern,$data)) {
				// Transaction is done,
				// Package up XML and return it
				// Remove the serverdone response and put an xcat tag around the rest
				$count = 0;
				$data = preg_replace($fullpattern,'', $data, -1, $count);		// 1st try to remove the long pattern
				if (!$count) { $data = preg_replace($mixedpattern,'', $data) . '</xcatresponse>/'; }		// if its not there, then remove the short pattern
				$data = "<xcat>$data</xcat>";
				...
				// Replace the '\n' by ':|:' in the data area
				$data = preg_replace('/\n/', ':|:', $data);
				$rsp = simplexml_load_string($data,'SimpleXMLElement', LIBXML_NOCDATA);
				$cleanexit = 1;
				break;
			}
		}
