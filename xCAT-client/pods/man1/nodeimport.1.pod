...
The B<nodeimport> command creates nodes by importing a hostinfo file which is following stanza format. In this hostinfo file, we can define node's hostname, ip, mac, switch name, switch port and host location information like rack, chassis, start unit, server height...etc
...
B<nicips=<nics-ip>> This is an optional item.
...
B<chassis=<chassis-name>> This is an optional item.
