...
Calling B<rinv> for VMware will display the UUID/GUID, number of CPUs, amount of memory, the MAC address and a list of Hard disks.  The output for each Hard disk includes the label, size and backing file location.
...
IMM, C<UEFI Version> reports the active firmware. If the IMM reports a pending
UEFI build ID, B<rinv> reports it separately as C<Pending UEFI Build ID>.
