...
B<lshwconn> - Use this command to display the connection status for CEC, Frame and IBM Flex Compute Nodes (Compute Node for short).
...
This command is used to display the connection status for CEC, Frame and Compute Nodes. 
...
The tooltype is used to communicate to the CEC/Frame/Compute Node. The value could be lpar or fnm. The tooltype value lpar is for xCAT and fnm is for CNM.
