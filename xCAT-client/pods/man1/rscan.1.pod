...
For the management module of blade, if the blade server is an IBM Flex System Compute Node (compute node for short), the fsp belongs to the blade server also will be scanned.
...
The Compute Node object of xCAT will be updated if the mpa and slot id are matched to the object which has been defined in the xCAT database.
