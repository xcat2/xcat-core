...
It is used to set up connections for CEC, Frame and IBM Flex Compute Node (Compute Node for short) to Hardware Server on management node (or service node ). It only could be done according to the node definition in xCAT DB. And this command will try to read the user/password from the ppcdirect table (for Compute Node, from mpa table) first. If fails, then read them from passwd table. 
...
To setup the connection between the Compute Node and hardware server:
