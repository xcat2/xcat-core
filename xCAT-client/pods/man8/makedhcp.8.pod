...
C<site.dhcpomapialgorithm> and C<site.dhcpomapikeyname>. If the system uses an
alternate ISC DHCP build, C<site.dhcpomshellpath> can point xCAT at that build's
C<omshell> binary. FIPS mode changes the default algorithm from C<hmac-md5> to
C<hmac-sha256> and rejects an explicit C<hmac-md5> value. On RHEL 8, SHA-based
OMAPI authentication requires C<dhcp-server> C<12:4.3.6-48> or later.
