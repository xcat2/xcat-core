...
xCAT uses **xcat_key** for BIND DDNS updates and legacy ISC DHCP OMAPI. New installations on Enterprise Linux 9 or later and Ubuntu 20.04 or later set **hmac-sha256**. Outside FIPS mode, an unset **dhcpomapialgorithm**, including on an existing installation, continues to use **hmac-md5** for compatibility. On a FIPS-enabled system, xCAT instead defaults to **hmac-sha256** and rejects an explicit **hmac-md5** setting. Ubuntu 18.04, SLES 12, SLES 15, and openSUSE Leap 15 normally leave this attribute unset because their bundled ``omshell`` does not support the ``key-algorithm`` command; a FIPS-mode site using OMAPI on those platforms must provide a compatible ``omshell``. Kea does not use OMAPI, but Kea DDNS uses this TSIG algorithm.
