...
sub getstate {
  ...
  if (check_dhcp($node)) {
    if (-r $tftpdir . "/armgrub/".$node) {
      my $fhand;
      open ($fhand,$tftpdir . "/armgrub/".$node);
      my $headline = <$fhand>;
      close $fhand;
      $headline =~ s/^#//;
      chomp($headline);
...
=pod
  ...
  This function will manipulate the armgrub structure to match what the noderes/chain tables indicate the node should be booting.
...
###hack end
  ...
  my $pcfg;
  unless (-d $tftpdir."/armgrub/") {
      mkpath($tftpdir."/armgrub/");
  }

  open($pcfg,'>',$tftpdir."/armgrub/".$node);
  ...
  if ($cref and $cref->{currstate} eq "boot") {
    ...
    print $pcfg " menuentry 'XCAT Install' --class os {\n    linux ".$kern->{kernel};
    if ($kern->{initrd} or $kern->{kcmdline}) {
      print $pcfg " " . $kern->{kcmdline}. ' BOOTIF=01-$mac1-$mac2-$mac3-$mac4-$mac5-$mac6'."\n";
    }
    print $pcfg "\n";
  ...
  foreach $ip (keys %ipaddrs) {
   ...
   unlink($tftpdir."/armgrub/grub.cfg-".$pname);
   if ($hassymlink) { 
    symlink($node,$tftpdir."/armgrub/grub.cfg-".$pname);
   } else {
    link($tftpdir."/armgrub/".$node,$tftpdir."/armgrub/grub.cfg-".$pname);
   }
  }
}
...
sub process_request {
  ...
  if($args[0] eq 'offline') {
    foreach my $node (@nodes) {
      ...
      unlink($tftpdir."/armgrub/".$node);

      foreach $ip (keys %ipaddrs) {
        my @ipa=split(/\./,$ip);
        my $pname = sprintf("%02X%02X%02X%02X",@ipa);
        unlink($tftpdir."/armgrub/".$pname);
        #if ($hassymlink) {
          #symlink($node,$tftpdir."/armgrub/".$pname);
        #} else {
          #link($tftpdir."/armgrub/".$node,$tftpdir."/armgrub/".$pname);
        #}
      }
    }
  }
