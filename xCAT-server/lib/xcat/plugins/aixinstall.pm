...
sub mkdsklsnode
{
    ...
    if ($::SETUPHANFS)
    {
		# get a list of all the cluster nodes  and all SNs
        ...
        my $sn = xCAT::ServiceNodeUtils->getSNformattedhash(\@nodel, "xcat", "MN", "primary");
        if ($::ERROR_RC)
        {
            my $rsp;
		...
		foreach my $s (@allSN) {
			if (!$xmast{$s} ) {
				...
				my @servlist;
				push @servlist, $s;
        		my $sint = xCAT::ServiceNodeUtils->getAIXSNinterfaces(\@servlist, $callback, $subreq);
        		my %sni = %$sint;		
