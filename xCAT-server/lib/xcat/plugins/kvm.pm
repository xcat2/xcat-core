...
sub rscan {
    ...
    #get 'node' and 'host' attributes from vm table...
    my %hash_vm2host;
    ...
    foreach $dom (@doms) {
        ...
        if (@vmmemoryobj and defined(@vmmemoryobj->[0])) {
            ...
            if ((@vmmemoryobj->[0]->getAttribute("unit") eq "KiB") or (@vmmemoryobj->[0]->getAttribute("unit") eq "k")) {
                $vmmemory=($mem*1024)/(1024*1024);
            }
            if (@vmmemoryobj->[0]->getAttribute("unit") eq "KB") {
                $vmmemory=($mem*1000)/(1024*1024);
            }
            if ((@vmmemoryobj->[0]->getAttribute("unit") eq "MiB") or (@vmmemoryobj->[0]->getAttribute("unit") eq "M")) {
                $vmmemory=$mem;
            }
            if (@vmmemoryobj->[0]->getAttribute("unit") eq "MB") {
                $vmmemory=($mem*1000000)/(1024*1024);
            }
            if ((@vmmemoryobj->[0]->getAttribute("unit") eq "GiB") or (@vmmemoryobj->[0]->getAttribute("unit") eq "G")) {
                $vmmemory=$mem*1024;
            }
            if (@vmmemoryobj->[0]->getAttribute("unit") eq "GB") {
                $vmmemory=($mem*1000000000)/(1024*1024);
            }
            if ((@vmmemoryobj->[0]->getAttribute("unit") eq "TiB") or (@vmmemoryobj->[0]->getAttribute("unit") eq "T")) {
                $vmmemory=$mem*1024*1024;
            }
            if (@vmmemoryobj->[0]->getAttribute("unit") eq "TB") {
        ...
        foreach my $vmstoragediskobj (@vmstoragediskobjs) {
            if (($vmstoragediskobj->getAttribute("type") eq "file") and ($vmstoragediskobj->getAttribute("device") eq "disk")) {
                my @vmstorageobj = $domain->findnodes("/domain/devices/disk/source");
        ...
        foreach my $interfaceobj (@interfaceobjs) {
            if (($interfaceobj->getAttribute("type")) eq "bridge" ) {
                my @vmnicsobj = $domain->findnodes("/domain/devices/interface/source");
                my @macobj = $domain->findnodes("/domain/devices/interface/mac");
                my @vmnicnicmodelobj = $domain->findnodes("/domain/devices/interface/model");
        ...
        if ($write) {
            unless (exists $hash_vm2host{$node}) {
            ...
            elsif ($hash_vm2host{$node} eq $hypervisor) {
                ...
                $callback->({data=>"$node guest information from $hypervisor conflict with the node from table."});
            }
        }
        if ($update) {
