...
sub process_request {
    ...
    if (%node_info) {
        my $node_json = encode_json \%node_info;
        my $node_info_file = "/opt/xcat/lib/perl/xCAT_plugin/node_info";
        open (NODE_INFO_FILE_HANDLE, "> $node_info_file");
        print NODE_INFO_FILE_HANDLE "$node_json\n";
        close (NODE_INFO_FILE_HANDLE);
           
        my $subcommands = join("_", @exargs);

        #Run opebmc.py $command_$subcommands

        my $result_file = "/opt/xcat/lib/perl/xCAT_plugin/python_rst";
        open (PYTHON_RST_FILE_HANDLE, "$result_file");
        while (my $line = <PYTHON_RST_FILE_HANDLE>) {
            my $response = decode_json $line;
            foreach my $key (keys %{ $response }) {
                if (ref($response->{$key}) eq "ARRAY") {
                    xCAT::MsgUtils->message("I", { data => ["$key: $_"] }, $callback) foreach (@{ $response->{$key} });
                } else {
                    xCAT::MsgUtils->message("I", { data => ["$key: $response->{$key}"] }, $callback);
                }
            }
        }
    }
