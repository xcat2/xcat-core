...
sub net_dns_uses_keyfile {
    my ($version) = @_;
    $version = Net::DNS->VERSION unless defined($version);
    return xCAT::Utils->version_cmp($version, "1.36") >= 0;
}
...
sub ddns_tsig_algorithm {
    ...
    return "hmac-md5"
      if (!net_dns_uses_keyfile($net_dns_version)
        && !$settings->{algorithm_enforced});
...
sub ddns_reconcile_key_algorithm {
    ...
    if ( $current
        && !net_dns_uses_keyfile($net_dns_version)
        && $current ne 'hmac-md5' )
    {
