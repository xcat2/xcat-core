#!/bin/bash
#
# The xCAT side of the provision-chain wire cases: blackboxtest knows nothing about xCAT,
...
stage_skip() {
    SKIPPED=$((SKIPPED + 1))
    echo "provfixture SKIP: ${STAGE:-fixture}: $*"
    return 0
}
...
# anywhere. Calls blackboxtest's own encoders rather than a second implementation in
...
from blackboxtest_lib import netutil
...
do_check() {
    ...
    # -f rather than -x: the fixture runs it as `python3 src/blackboxtest`, and
...
prov_run() {
    ( cd "$TOOL" && in_ns python3 src/blackboxtest run "$@" )
}
...
do_run_tftp() {
    ...
    prov_run \
        ...
        conf/prov/tftp-pxelinux.conf || rc=1
...
do_run_genesis() {
    ...
    prov_run $select \
        ...
        conf/prov/discovery-artefacts.conf
}
...
do_run_discovery() {
    ...
    prov_run \
        "${COMMON[@]}" \
        conf/prov/flowcontrol.conf || rc=1

    prov_run \
        "${COMMON[@]}" \
        --set foreign="$FOREIGN_IP" \
        conf/prov/findme.conf || rc=1
    return $rc
}
...
do_run_xcatd() {
    ...
    prov_run \
        ...
        conf/prov/xcatd-policy.conf || rc=1
    ...
    prov_run \
        ...
        conf/prov/xcatd-postscript.conf || rc=1

    prov_run \
        "${COMMON[@]}" \
        --set xcatport="$XCATPORT" --set credtype=xcat_server_cred \
        --set highport="$HIGHPORT" \
        conf/prov/xcatd-credentials.conf || rc=1
    ...
    prov_run \
        ...
        conf/prov/xcatd-destiny.conf || rc=1
    return $rc
}
...
do_run_ordering() {
    ...
    prov_run -s unreachable-master "${flags[@]}" conf/prov/ordering.conf || rc=1
    ...
    if ! have_genesis; then
        ...
        prov_run -s state-replaced "${flags[@]}" conf/prov/ordering.conf || rc=1
    fi
