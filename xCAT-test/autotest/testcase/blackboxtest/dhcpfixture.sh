#!/bin/bash
#
# The xCAT side of the DHCP wire cases: blackboxtest knows nothing about xCAT,
...
#     dhcpfixture.sh run                     run blackboxtest against it
...
#     dhcpfixture.sh run-hierarchy           run blackboxtest against the delegated network
...
TOOL=/opt/xcat/share/xcat/tools/autotest/blackboxtest
[ -d "$TOOL" ] || TOOL="$(cd "$(dirname "$0")/../../../blackboxtest" 2>/dev/null && pwd)"
...
# one: blackboxtest asserts the name in the BOOTP file field and never opens a TFTP
...
do_check() {
    ...
    # -f rather than -x: the fixture runs it as `python3 src/blackboxtest`. Testing
    # -x made every wire case skip -- and pass -- on Debian, where dh_install
    # keeps the source mode.
    [ -f "$TOOL/src/blackboxtest" ] || skip "blackboxtest is not installed under $TOOL"
...
dhcp_run() {
    ( cd "$TOOL" && python3 src/blackboxtest run -i "$IF_CLI" "$@" )
}
...
do_run() {
    ...
    dhcp_run \
        ...
        conf/dhcp/provision-vs-discovery.conf || rc=1
...
do_run_arch() {
    dhcp_run --set tftp="$SRV_IP" \
        ...
        conf/dhcp/pxe-arch-matrix.conf
}
...
do_run_netboot() {
    ...
    dhcp_run \
        ...
        conf/dhcp/netboot-methods.conf
}
...
do_run_lease() {
    local rc=0
    dhcp_run --set net="$NET/$PREFIX" conf/dhcp/full-lease.conf   || rc=1
    dhcp_run --set net="$NET/$PREFIX" conf/dhcp/renew-rebind.conf || rc=1
    dhcp_run --set foreign_ip="$FOREIGN_IP" conf/dhcp/nak-foreign-address.conf || rc=1
    return $rc
}
...
do_run_adoption() {
    ...
    dhcp_run -s machine-is-unknown \
        --set adopt_mac="$ADOPT_MAC" --set adopt_ip="$ADOPT_IP" --set pool="$POOL" \
        conf/dhcp/discovery-adoption.conf || rc=1
    ...
    dhcp_run -s machine-has-been-adopted \
        --set adopt_mac="$ADOPT_MAC" --set adopt_ip="$ADOPT_IP" --set pool="$POOL" \
        conf/dhcp/discovery-adoption.conf || rc=1
...
do_run_nextserver() {
    ...
    dhcp_run \
        ...
        conf/dhcp/next-server-source.conf || rc=1
    return $rc
}
...
do_run_multimac() {
    ...
    dhcp_run \
        ...
        conf/dhcp/multi-mac-node.conf || rc=1
    return $rc
}
...
do_run_iscsi() {
    ...
    dhcp_run \
        ...
        conf/dhcp/iscsi-root-path.conf || rc=1
    return $rc
}
...
do_run_loader_absent() {
    ...
    if do_generate; then
        dhcp_run --set pool="$POOL" --set present_loader="$(arch_loader uefi)" \
            conf/dhcp/loader-absent.conf || rc=1
    else
...
do_run_httpport() {
    ...
    if do_generate; then
        dhcp_run --set tftp="$SRV_IP" --set httpport="$ALT_HTTPPORT" \
            --set riscv64_loader="$(arch_loader riscv64)" \
            conf/dhcp/http-port.conf || rc=1
    else
...
do_run_rangecidr() {
    ...
    if do_generate; then
        dhcp_run --set unknown_mac="$UNKNOWN_MAC" --set pool="$POOL_CIDR" \
            conf/dhcp/dynamic-range-cidr.conf || rc=1
    else
...
do_run_removal() {
    ...
    dhcp_run -s removed-node-keeps-its-reservation-until-it-is-withdrawn \
        --set removed_mac="$RM_MAC" --set removed_ip="$RM_IP" --set pool="$POOL" \
        conf/dhcp/node-removal.conf || rc=1

    makedhcp -d "$RM_NODE" || die "makedhcp -d $RM_NODE failed"

    dhcp_run -s withdrawn-node-is-no-longer-served-its-address \
        --set removed_mac="$RM_MAC" --set removed_ip="$RM_IP" --set pool="$POOL" \
        conf/dhcp/node-removal.conf || rc=1
    return $rc
}
...
do_run_bootp() {
    dhcp_run --set bootp_mac="$BOOTP_MAC" --set pool="$POOL" \
        conf/dhcp/bootp-client.conf
}

do_run_hierarchy() {
    dhcp_run --set node_mac="$NODE_MAC" --set node_ip="$NODE_IP" \
        --set node_loader="$(node_loader)" \
        --set delegate="$DELEGATE_IP" --set unknown_mac="$UNKNOWN_MAC" \
        conf/dhcp/hierarchy-dhcpserver.conf
}
