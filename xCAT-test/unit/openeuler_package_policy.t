...
foreach my $role (@roles) {
    ...
    BAIL_OUT("Unable to parse $spec: $requires") if $rc;
...
foreach my $scenario (@scenarios) {
    ...
    BAIL_OUT("Cannot initialize test RPM database: $init_output") if $init_rc;
    my ($install_rc, $install_output) = run('rpm', '--dbpath', $db, '--justdb', '--nodeps', '-i', map {$provider{$_}} @providers);
    BAIL_OUT("Cannot seed test RPM database: $install_output") if $install_rc;
    foreach my $role (@roles) {
...
sub build_fixture {
    ...
    BAIL_OUT("Cannot build RPM fixture $name: $output") if $rc;
    my $rpm = "$root/RPMS/noarch/$name-1-1.noarch.rpm";
    BAIL_OUT("RPM fixture missing: $rpm") unless -f $rpm;
    return $rpm;
}
sub run {
    my (@command) = @_;
