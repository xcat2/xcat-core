...
TAP is the default because `prove` consumes it directly, and because it is what
dhcptest emits, so a driver that already collects one suite's output collects
this one unchanged.

The failure diagnostic carries expected, received, the reply's fields and the
command or request that produced it, so a red test can be understood from the
log without re-running it -- which matters most in CI, where the network it ran
on no longer exists by the time anyone reads the result.
"""
...
class PrettyReporter(Reporter):
    ...
    def finish(self):
        self.write("\nPROVTEST SUMMARY: total=%d passed=%d failed=%d skipped=%d\n"
...
def _diagnostic_lines(record):
    ...
    if record.detail:
        lines.append("detail:   %s" % (_oneline(record.detail),))
    if record.reply is not None:
        lines.append("reply:    %s" % (_oneline(record.reply.summary()),))
    else:
        lines.append("reply:    none received")
    if record.sent:
        lines.append("sent:     %s" % (_oneline(record.sent),))
    if record.attempts:
...
def _oneline(text, limit=400):
    """Collapse a value to one line, because TAP's diagnostics are per line."""
