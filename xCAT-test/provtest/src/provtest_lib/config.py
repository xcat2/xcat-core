...
by the library, out of `[vars]` and of anything passed with `--set`. `$` has no
meaning to it, so `$grubcfg.text` reaches the runtime untouched -- and it has
to, because its value is a reply that has not arrived when the file is read.
...
SCENARIO_KEYS = frozenset(["description"])

#: Keys `[defaults]` may carry. `type` and `assert` are per-step by nature: a
#: default type would make a mistyped `type=` silently become another protocol,
#: and a default assertion would be made about replies it was never written for.
DEFAULTS_KEYS = frozenset(
    key
    for step_type in machine.STEP_KEYS
    for key in machine.allowed_keys(step_type)
) - frozenset(["type", "assert", "description"])
    ...
    for section in parser.sections():
        ...
        if kind == "scenario":
            ...
            if current is not None:
                scenarios.append(_build_scenario(current, steps, path))
...
def _build_scenario(current, steps, path):
    name, items = current
    if not steps:
...
def _build_step(name, items, defaults, where):
    ...
    machine.check_type(step_type, where)

    allowed = machine.allowed_keys(step_type)
    _reject_unknown(items, allowed, where)

    # A default only applies where the type in hand can use it, so one
    # `[defaults] server =` serves dns, tftp, http and xcatreq steps alike
    # without making `path =` illegal for the dns step in the same file.
    merged = dict((key, value) for key, value in defaults.items()
                  if key in allowed)
    merged.update(items)

    expect = (merged.get("expect") or "").strip().lower() or None
    timeout = _float(merged.get("timeout"), 5.0, "timeout", where)
    retries = _int(merged.get("retries"), 2, "retries", where)
...
#: -- also make each variable an *option*, so `--set path=/install` would
