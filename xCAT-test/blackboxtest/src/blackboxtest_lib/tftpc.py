...
def fetch(server, path, port=69, mode="octet", timeout=10.0):
    """Fetch one file and return a `Reply` describing what arrived."""
    handle, local = tempfile.mkstemp(prefix="blackboxtest-tftp-")
    os.close(handle)
    try:
        argv = build(server, path, port, mode, local)
        done = proc.run(argv, timeout=float(timeout) + 5.0)
        return _decode(done, local, server, path)
    finally:
        try:
            os.unlink(local)
        except OSError:
...
def _decode(done, local, server, path):
    ...
    return Reply(kind="tftp", fields=fields, ok=ok, error=error,
                 sent=done.command())
