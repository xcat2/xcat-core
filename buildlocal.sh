...
. "$CURDIR/build-utils/rpm-architectures.sh"
...
echo "This is an $OSNAME system"
     ...
     if [ "$OS" = "SUSE" ]; then
         for arch in $XCAT_LOCAL_COLLECT_ARCHES; do
             rm -rf /usr/src/packages/RPMS/$arch/*
         done
     else
         for arch in $XCAT_LOCAL_COLLECT_ARCHES; do
             rm -rf /root/rpmbuild/RPMS/$arch/*
         done
     fi
  ...
  for rpmname in xCAT xCATsn; do
                if [ "$OSNAME" = "AIX" ]; then
                        ...
                        for arch in $XCAT_LOCAL_RPM_ARCHES; do
  ...
  if [ "$OS" = "SUSE" ]; then
      for arch in $XCAT_LOCAL_COLLECT_ARCHES; do
          cp /usr/src/packages/RPMS/$arch/* $CURDIR/build/
      done
  else
      for arch in $XCAT_LOCAL_COLLECT_ARCHES; do
          cp /root/rpmbuild/RPMS/$arch/* $CURDIR/build/
      done
  fi
